...
class THTConvNeXtAttnConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(THTConvNeXtAttnConfig, self).__init__(**kwargs)


class THTConvNeXtAttn(PreTrainedModel):
    config_class = THTConvNeXtAttnConfig

    def __init__(self, config):
        super(THTConvNeXtAttn, self).__init__(config)
        ...
        self.extractor1 = create_feature_extractor(
            convnext1, return_nodes={"features.7.2.add": "features"}
        )
        self.extractor2 = create_feature_extractor(
            convnext2, return_nodes={"features.7.2.add": "features"}
        )

        self.embed_dim = convnext1.classifier[2].in_features
...
if __name__ == "__main__":
    ...
    config = THTConvNeXtAttnConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = THTConvNeXtAttn(config).to(device)
    print(model)
