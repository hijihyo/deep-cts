...
class OnlyTConvNeXtConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(OnlyTConvNeXtConfig, self).__init__(**kwargs)


class OnlyTConvNeXt(PreTrainedModel):
    config_class = OnlyTConvNeXtConfig

    def __init__(self, config):
        super(OnlyTConvNeXt, self).__init__(config)

        self.convnext = getattr(models, config.variant)(weights=config.weights, progress=True)
        if config.num_classes != 1000:
            self.convnext.classifier[2] = nn.Linear(
                self.convnext.classifier[2].in_features, config.num_classes
            )
            nn.init.trunc_normal_(self.convnext.classifier[2].weight, std=0.02)
            if self.convnext.classifier[2].bias is not None:
                nn.init.zeros_(self.convnext.classifier[2].bias)
...
if __name__ == "__main__":
    ...
    config = OnlyTConvNeXtConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = OnlyTConvNeXt(config).to(device)
    print(model)
