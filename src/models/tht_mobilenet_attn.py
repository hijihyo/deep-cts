...
class THTMobileNetAttnConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(THTMobileNetAttnConfig, self).__init__(**kwargs)


class THTMobileNetAttn(PreTrainedModel):
    config_class = THTMobileNetAttnConfig

    def __init__(self, config):
        super(THTMobileNetAttn, self).__init__(config)
        ...
        self.extractor1 = create_feature_extractor(mnet1, return_nodes={"features.12": "features"})
        self.extractor2 = create_feature_extractor(mnet2, return_nodes={"features.12": "features"})

        self.embed_dim = mnet1.features[12][0].out_channels
...
if __name__ == "__main__":
    ...
    config = THTMobileNetAttnConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = THTMobileNetAttn(config).to(device)
    print(model)
