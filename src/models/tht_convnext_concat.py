...
class THTConvNeXtConcatConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(THTConvNeXtConcatConfig, self).__init__(**kwargs)


class THTConvNeXtConcat(PreTrainedModel):
    config_class = THTConvNeXtConcatConfig

    def __init__(self, config):
        super(THTConvNeXtConcat, self).__init__(config)

        convnext1 = getattr(models, config.variant)(weights=config.weights, progress=True)
        convnext2 = getattr(models, config.variant)(weights=config.weights, progress=True)

        self.extractor1 = create_feature_extractor(
            convnext1, return_nodes={"classifier.1": "features"}
        )
        self.extractor2 = create_feature_extractor(
            convnext2, return_nodes={"classifier.1": "features"}
        )

        self.classifier = nn.Linear(convnext1.classifier[2].in_features * 2, config.num_classes)
...
if __name__ == "__main__":
    ...
    config = THTConvNeXtConcatConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = THTConvNeXtConcat(config).to(device)
    print(model)
