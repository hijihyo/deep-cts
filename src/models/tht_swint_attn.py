...
class THTSwinTAttnConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(THTSwinTAttnConfig, self).__init__(**kwargs)


class THTSwinTAttn(PreTrainedModel):
    config_class = THTSwinTAttnConfig

    def __init__(self, config):
        super(THTSwinTAttn, self).__init__(config)
        ...
        self.extractor1 = create_feature_extractor(swint1, return_nodes={"permute": "features"})
        self.extractor2 = create_feature_extractor(swint2, return_nodes={"permute": "features"})

        self.embed_dim = swint1.head.in_features
...
if __name__ == "__main__":
    ...
    config = THTSwinTAttnConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = THTSwinTAttn(config).to(device)
    print(model)
