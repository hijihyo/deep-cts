...
class OnlyTMobileNetConfig(PretrainedConfig):
    def __init__(
        self,
        variant: str = "default",
        weights: str = "none",
        ...
        **kwargs,
        ...
        super(OnlyTMobileNetConfig, self).__init__(**kwargs)


class OnlyTMobileNet(PreTrainedModel):
    config_class = OnlyTMobileNetConfig

    def __init__(self, config):
        super(OnlyTMobileNet, self).__init__(config)

        self.mnet = getattr(models, config.variant)(weights=config.weights, progress=True)
        if config.num_classes != 1000:
            self.mnet.classifier[3] = nn.Linear(
                self.mnet.classifier[3].in_features, config.num_classes
            )
            nn.init.normal_(self.mnet.classifier[3].weight, 0, 0.01)
            nn.init.zeros_(self.mnet.classifier[3].bias)
...
if __name__ == "__main__":
    ...
    config = OnlyTMobileNetConfig(args.variant, args.weights, args.num_classes)
    device = torch.device(args.device)
    model = OnlyTMobileNet(config).to(device)
    print(model)
