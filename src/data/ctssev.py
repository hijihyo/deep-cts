...
class CTSSev(Dataset):
    """PyTorch Dataset for CTS severity prediction"""
    ...
    def __getitem__(
        ...
        severity = ["normal", "mild", "moderate", "severe"].index(self.severities[index])
        ...
        if self.target_transform:
            severity = self.target_transform(severity)

        return {"inputs1": t_img, "inputs2": ht_img, "labels": severity}
...
if __name__ == "__main__":
    ...
    dataset = CTSSev(args.root, args.split)
    print(dataset)
    ...
    severity = dataset[args.index]["labels"]
    print("Data Example")
    print("  Thenar image:", t_img.size, t_img.mode)
    print("  Hypothenar image:", ht_img.size, ht_img.mode)
    print("  Severity:", severity, (["normal", "mild", "moderate", "severe"])[severity])
